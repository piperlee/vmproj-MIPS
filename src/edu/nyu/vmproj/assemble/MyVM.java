...
public class MyVM {
  // map: instr addr -> instr
	...
	// profile: dbbMap(dynamic basic block map): dbb -> executing times
	Profile profile;
	int curDBBStart;
	int curDBBEnd;
	//TODO: for test
    ...
    if (op.equals("call")) {      
      ...
      curDBBEnd = regMap.get("eip");
      profile.updateDBBMap(new DBB(curDBBStart,curDBBEnd));
      setEIP(labelMap.get(inst.getArg1()));
      curDBBStart = regMap.get("eip");
      return;
    }
	...
	public void run(String fileName) {
		...
		// print out profile statistics
		curDBBEnd = regMap.get("eip")-4;
		if (curDBBEnd >= curDBBStart) {
		  DBB dbb = new DBB(curDBBStart,curDBBEnd);
		  if (!profile.contains(dbb)) {
		    profile.updateDBBMap(dbb);
		  }		  
		}   
		profile.printDBBMap();
	}
	...
	public static void main(String[] args) throws IOException {
		if (args.length < 1) {
			...
			MyVM vm = new MyVM();
			//TODO: for test
			MyVM.out = new PrintWriter(new BufferedWriter(new FileWriter(fileName + ".out")));
			vm.run(fileName);
			MyVM.out.close();
		}
	}
